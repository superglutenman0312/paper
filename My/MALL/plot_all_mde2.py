...
def get_wd_mde(seed, mode, alpha, beta):
    """
    取得 WD 特定參數下的 MDE
    路徑結構: WD/experiments4/random_seed_{seed}/{alpha}_{beta}_{epoch}_{mode}/predictions/
    """
    folder_name = f"{float(alpha)}_{float(beta)}_{EPOCH_SWD}_{mode}"
    # 修改: 使用 WD_BASE_DIR
...
def plot_combined_comparison(df, title_seed_part, output_filename):
    ...
    fig, axes = plt.subplots(1, 2, figsize=(20, 7)) # 寬度稍微拉大一點以容納更多 Bar
    ...
    for i, mode in enumerate(modes):
        ...
        ax.set_title(f'{mode.capitalize()} Mode')
        ax.set_xticks(x)
        ax.set_xticklabels(plot_names, rotation=45, ha='right') # 角度稍微加大避免重疊
