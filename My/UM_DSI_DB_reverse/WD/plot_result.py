...
def main():
    ...
    if EXPERIMENT_TYPE == 'time_reversal_1':
        SOURCE_FILE_TAG = '200219' # 假設 source tag 不變
        TARGET_FILE_TAG = '190611' # 假設 target tag 不變
        print(f"開始分析 [time_reversal_1] 實驗 (Source: {SOURCE_FILE_TAG}, Target: {TARGET_FILE_TAG})")
    elif EXPERIMENT_TYPE == 'time_reversal_2':
        SOURCE_FILE_TAG = '200219' # 假設 source tag 不變
        TARGET_FILE_TAG = '191009' # 假設 target tag 不變
        print(f"開始分析 [time_reversal_2] 實驗 (Source: {SOURCE_FILE_TAG}, Target: {TARGET_FILE_TAG})")
    else:
