...
def get_wd_errors(exp_type, seed, mode, alpha, beta, tgt_file):
    folder_name = f"{float(alpha)}_{float(beta)}_{EPOCH_SWD}_{mode}"
    model_dir = os.path.join(WD_ROOT_NAME, exp_type, f'random_seed_{seed}', folder_name, 'predictions')
...
def main():
    ...
    print(f"正在載入座標表: {MAP_FILE_PATH} ...")
    LABEL_TO_COORDINATE_DICT = load_coordinate_map(MAP_FILE_PATH)
    
    if not LABEL_TO_COORDINATE_DICT:
        print("警告: 座標表為空或載入失敗，計算出的 MDE 可能皆為 NaN。")
    ...
    for exp_type, (src_file, tgt_file) in SCENARIOS.items():
        ...
        # 結構: cdf_global_data[mode][method_name] = [err1, err2, ...]
        ...
        for seed in SEEDS:
            ...
            for mode in ['labeled', 'unlabeled']:
                ...
                for (alpha, beta, name) in WD_VARIANTS:
                    ...
                    # === 新增: 收集 WD Errors 用於 CDF ===
                    errs = get_wd_errors(exp_type, seed, mode, alpha, beta, tgt_file)
                    if errs:
