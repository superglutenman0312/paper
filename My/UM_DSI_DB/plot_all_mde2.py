...
# 4. 定義其他方法 (對應資料夾名稱)
#    請確認這些資料夾存在於根目錄下
OTHER_METHODS = ["DANN", "DANN_CORR", "DANN_CORR_GEMINI", "DNN"]

# 5. 定義實驗場景 (對應資料夾名稱與檔案標籤)
...
# 6. 輸出目錄
OUTPUT_DIR = "Comparison_Result_TV_test2"

# 7. 根目錄名稱設定
SWD_ROOT_NAME = "SWD"
WD_ROOT_NAME = "WD"  # 新增 WD 根目錄
...
def get_wd_mde(exp_type, seed, mode, alpha, beta, src_file, tgt_file):
    """
    取得 WD 的 MDE (新增)
    路徑: WD/{exp_type}/random_seed_{seed}/{alpha}_{beta}_{epoch}_{mode}/predictions/
    """
    folder_name = f"{float(alpha)}_{float(beta)}_{EPOCH_SWD}_{mode}"
    # WD 位於 WD/time_variation_1/...
...
def plot_combined_comparison(df, title_seed_part, exp_name, output_filename):
    ...
    fig, axes = plt.subplots(1, 2, figsize=(20, 7)) # 加寬以容納 WD
