r'''
# time variation 1
python SWD2.py --training_source_domain_data D:\paper_thesis\Histloc_real\Experiment\data\UM_DSI_DB_v1.0.0_lite\data\tony_data\2019-06-11\wireless_training.csv `
...
python SWD2.py --test --work_dir time_variation_1 `
             --loss_weights 0.1 10 --epoch 5 --random_seed 42 --unlabeled
...
python SWD2.py --test --work_dir time_variation_2 `
             --loss_weights 0.1 10 --epoch 20 --random_seed 42 --unlabeled
...
class FeatureExtractor(nn.Module):
    def __init__(self, input_size, hidden_size1, hidden_size2):
        ...
        self.bn1 = nn.BatchNorm1d(hidden_size1)  # 加入 BN
        self.fc2 = nn.Linear(hidden_size1, hidden_size2)
        self.bn2 = nn.BatchNorm1d(hidden_size2)  # 加入 BN，確保輸出的 scale 一致

    def forward(self, x):
        x = self.fc1(x)
        x = self.bn1(x)  # 先 BN 再 ReLU，或者是先 ReLU 再 BN 都可以(通常建議先BN)
        x = torch.relu(x)
        x = self.fc2(x)
        x = self.bn2(x)  # 這裡最重要，這層輸出會直接拿去算 SWD
        x = torch.relu(x)
        return x
