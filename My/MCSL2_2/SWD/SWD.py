'''
# time variation
python SWD.py --training_source_domain_data D:\paper_thesis\My\data\MCSL\processed_data\20220318_20231116\source_train.csv `
             --training_target_domain_data D:\paper_thesis\My\data\MCSL\processed_data\20220318_20231116\target_train.csv `
             --work_dir time_variation `
             --loss_weights 1 10 --random_seed 42 --case 1 --unlabeled
python SWD.py --test --work_dir time_variation `
             --loss_weights 1 10 --random_seed 42 --case 1 --unlabeled

# spatial variation
python SWD.py --training_source_domain_data D:\paper_thesis\My\data\MCSL\processed_data\20231116_20231117\source_train.csv `
             --training_target_domain_data D:\paper_thesis\My\data\MCSL\processed_data\20231116_20231117\target_train.csv `
             --work_dir spatial_variation `
             --loss_weights 1 10 --random_seed 42 --case 2 --unlabeled
python SWD.py --test --work_dir spatial_variation `
             --loss_weights 1 10 --random_seed 42 --case 2 --unlabeled
'''
import torch
...
class HistCorrDANNModel:
    ...
    def _initialize_projections(self):
        ...
        if (self.feature_dim & (self.feature_dim - 1) == 0) and self.feature_dim != 0:
            # 這裡原本就有 .to(self.device)，現在 self.device 有定義了所以沒問題
            h_mat = self._get_hadamard_matrix(self.feature_dim).to(self.device)
            # [修改] 加上 device=self.device 確保 tensor 在同一裝置
        ...
        while current_count < self.num_projections:
            # 1. 生成隨機正交基底 (Base)
            # 這裡原本就有 device=self.device，現在 self.device 有定義了所以沒問題
